...
int64 nChainStartTime = 1544645118;
...
const string strMessageMagic = "Zarbitfast Signed Message:\n";
...
{
	int64 nSubsidy = 12000 * COIN;

        if (nHeight < 350)
        {
        nSubsidy = 1000000 * COIN;  // premine 100 mill to Zarbit holders and 250 mill to Zarbitfast investments
        ...
        if (nHeight < 8000)
        {
        nSubsidy = 30 * COIN;  // Slow starter mining
        return nSubsidy + nFees;   
        }
        

    // Subsidy is cut in half every 85000 blocks, which will occur approximately every 6 months 
    nSubsidy >>= (nHeight / 85000);
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Zarbitfast", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        // Genesis block
        const char* pszTimestamp = "Birth of the weirdo Zarfast coin or its christmas 2018 right now without a life";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1545690641;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2084762543;
        ...
        assert(block.hashMerkleRoot == uint256("0xfc4c03dd7866ea9f826ad34dcda1b905326cc8d8b56504cfce1b7084bf9bdb92"));
...
unsigned char pchMessageStart[4] = { 0x1a, 0x4f, 0x23, 0x28 };
    ...
    if (strCommand == "version")
    {
        ...
        if (!vRecv.empty())
        {
          ...
          if(nTime < VERSION4_SWITCH_TIME)
            iSubVer=pfrom->strSubVer.find("Zarbitfast:3");
          else
            iSubVer=pfrom->strSubVer.find("Zarbitfast:3.1");
